  ...
  if (model != "lm" & model != "logistic" & model != "felm" & model != "lme") {
    ...
    res <- foreach(i = 1:nrow(specs), .combine='rbind') %dopar% {
      mod <- summary(lfe::felm(as.formula( #as.formula() allows us to paste things into a formula
        ...
        data=data))
      res <- c(ifelse(specs$fixed.effects[i] != '0', mod$coefficients[1,1], mod$coefficients[2,1]), #estimate
               ifelse(specs$fixed.effects[i] != '0', mod$coefficients[1,1] - 1.96 * mod$coefficients[1,2],
                      mod$coefficients[2,1] - 1.96 * mod$coefficients[2,2]), #Lower-bound
               ifelse(specs$fixed.effects[i] != '0', mod$coefficients[1,1] + 1.96 * mod$coefficients[1,2], #upper-bound
                      mod$coefficients[2,1] + 1.96 * mod$coefficients[2,2]),
               ifelse(specs$fixed.effects[i] != '0', mod$coefficients[1,3], mod$coefficients[2,3]), #t val
               ifelse(specs$fixed.effects[i] != '0', mod$coefficients[1,4], mod$coefficients[2,4]), # p val
               mod$r.squared)
    }
    # Now set the results
    specs$B1.coef <- res[,1]
