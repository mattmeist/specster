...
plot_specster <- function(specs,
                          ...
                          return = 'both',
                          h0 = 0,
                          choice.colors = c('black', 'grey20', 'grey45', 'grey70'),
  ...
  if (return != 'curve') {
    ...
    #fixed-effect linear
    if(model == 'felm'){
      if(length(choice.colors) != 4){
        stop("For felm, you must supply four colors to the choice.colors argument.")
      }
      ...
      choices <- specs %>%
        ...
        pivot_longer(c(all_of(controls.vec), 
                       ...
                       all_of(data.vec)), 'Variable') %>% # Call variables variable
        mutate(Variable = as.factor(Variable), #Set Variable to factor
               ...
               Purpose = factor(ifelse(Variable %in% controls.vec, 'Controls',
                                       ifelse(Variable %in% fixed.effects.vec, 'Fixed Effects',
                                              ifelse(Variable %in% cluster.se.vec, 'Clustering', 'Data'))),
                                levels = c('Data', 'Fixed Effects', 'Controls', 'Clustering'))) %>% 
      ...
      if(model == 'lme'){
        if(length(choice.colors) != 4){
          stop("For lme, you must supply four colors to the choice.colors argument.")
        }
          ...
          mutate(Variable = as.factor(Variable), #Set Variable to factor
                 ...
                 Purpose = factor(ifelse(Variable %in% controls.vec, 'Controls',
                                         ifelse(Variable %in% random.slopes.vec, 'Random Slopes',
                                                ifelse(Variable %in% random.intercepts.vec, 'Random Intercepts', 'Data'))),
                                  levels = c('Data', 'Random Slopes', 'Random Intercepts', 'Controls'))) %>% 
          mutate(Variable = as.factor(stringr::str_remove(Variable, '\\.\\w*$'))) %>%
          as.data.table() %>%
          ggplot(aes(h.order, Variable, color = Purpose)) +
          facet_grid(Purpose~., scales = 'free', space = 'free') +
        ...
        {
          if(length(choice.colors) != 2){
            stop("For lm/logit, you must supply two colors to the choice.colors argument.")
          }
          # Set the controls
          controls.vec <- stringr::str_extract_all(specs$controls, '\\w+') %>%
            unlist() %>%
          ...
          choices <- specs %>%
            ...
            mutate(Variable = as.factor(Variable), #Set Control to factor
                   ...
                   Purpose = factor(ifelse(Variable %in% controls.vec, 'Controls', 'Data'),
                                    levels = c('Data', 'Controls'))) %>%
            mutate(Variable = as.factor(stringr::str_remove(Variable, '\\.\\w*$'))) %>%
            as.data.table() %>%
            ...
            theme(strip.text.y = element_text(size = 14, angle = 0)) + 
            geom_text(aes(label = value)) +
            scale_color_manual(values = choice.colors[1:2]) +
