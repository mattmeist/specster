...
run_specster <- function(data, 
                         specs, 
                         model, 
                         cores = 3,
                         num.specs = NULL){
  
  # Subset to a certain number of specifications
  specs <- ifelse(is.null(num.specs), specs, 
                  ifelse(num.specs >= nrow(specs), specs, 
                         specs[sample(num.specs)]))
